...
def test_cellpose_split_can_split_pairlike_small_merge_with_two_internal_peaks():
    single = np.zeros((90, 90), dtype=bool)
    single[34:50, 10:22] = True

    merged = np.zeros((90, 90), dtype=bool)
    rr, cc = np.ogrid[:90, :90]
    left = ((rr - 45) ** 2) / (9**2) + ((cc - 40) ** 2) / (7**2) <= 1.0
    right = ((rr - 45) ** 2) / (9**2) + ((cc - 53) ** 2) / (7**2) <= 1.0
    bridge = ((rr - 45) ** 2) / (7**2) + ((cc - 46) ** 2) / (9**2) <= 1.0
    merged[left | right | bridge] = True

    frame = _components_from_masks([single, merged])
    cfg = AppConfig()
    cfg.detector.cellpose_overlap_split_area_ratio = 99.0
    cfg.detector.cellpose_overlap_split_combo_area_ratio = 99.0
    cfg.detector.cellpose_overlap_split_pairlike_min_area_ratio = 1.0
    cfg.detector.cellpose_overlap_split_pairlike_max_area_ratio = 2.2
    cfg.detector.cellpose_overlap_split_pairlike_min_aspect_ratio = 1.5
    cfg.detector.cellpose_overlap_split_pairlike_max_aspect_ratio = 4.0
    cfg.detector.cellpose_overlap_split_pairlike_min_eccentricity = 0.75
    cfg.detector.cellpose_overlap_split_pairlike_max_extent = 0.85
    cfg.detector.cellpose_overlap_split_pairlike_peak_min_distance = 4
    cfg.detector.cellpose_overlap_split_pairlike_peak_abs_threshold = 0.30
    cfg.detector.cellpose_overlap_split_pairlike_min_child_area_ratio = 0.18
    cfg.detector.cellpose_overlap_split_pairlike_max_child_area_ratio = 2.5

    guide = np.full((90, 90, 3), 255, dtype=np.uint8)
    cv_mask = merged.astype(np.uint8) * 255
    guide[cv_mask > 0] = np.array([212, 176, 138], dtype=np.uint8)
    guide[38:54, 34:42] = np.array([135, 88, 48], dtype=np.uint8)
    guide[38:54, 51:59] = np.array([135, 88, 48], dtype=np.uint8)
...
def test_cellpose_split_pairlike_can_choose_best_pair_from_three_peaks():
    single = np.zeros((90, 90), dtype=bool)
    single[34:50, 10:22] = True

    merged = np.zeros((90, 90), dtype=bool)
    rr, cc = np.ogrid[:90, :90]
    left = ((rr - 45) ** 2) / (9**2) + ((cc - 38) ** 2) / (7**2) <= 1.0
    right = ((rr - 45) ** 2) / (9**2) + ((cc - 56) ** 2) / (7**2) <= 1.0
    bridge = ((rr - 45) ** 2) / (7**2) + ((cc - 47) ** 2) / (9**2) <= 1.0
    merged[left | right | bridge] = True
    ...
    cfg.detector.cellpose_overlap_split_pairlike_min_eccentricity = 0.75
    cfg.detector.cellpose_overlap_split_pairlike_max_extent = 0.85
    cfg.detector.cellpose_overlap_split_pairlike_peak_min_distance = 3
    cfg.detector.cellpose_overlap_split_pairlike_peak_abs_threshold = 0.25
    cfg.detector.cellpose_overlap_split_pairlike_min_child_area_ratio = 0.18
    cfg.detector.cellpose_overlap_split_pairlike_max_child_area_ratio = 2.5

    guide = np.full((90, 90, 3), 255, dtype=np.uint8)
    guide[merged] = np.array([212, 176, 138], dtype=np.uint8)
    guide[38:54, 32:40] = np.array([135, 88, 48], dtype=np.uint8)
    guide[38:54, 49:57] = np.array([135, 88, 48], dtype=np.uint8)
    guide[43:50, 42:46] = np.array([150, 100, 60], dtype=np.uint8)
